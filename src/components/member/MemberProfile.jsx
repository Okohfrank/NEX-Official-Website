...
export const MemberProfile = () => {
  const { currentUser, updateUserProfile, certificates, leaderboard, showToast } = useApp();
  const [isEditing, setIsEditing] = useState(false);
  const fileInputRef = useRef(null);

  // Form State
  const [name, setName] = useState(currentUser.name || 'George Ikechukwu');
  const [dept, setDept] = useState(currentUser.dept || 'Faculty of Engineering (Mechanical)');
  ...
  const handleSaveProfile = (e) => {
    ...
    if (selectedFocus.length === 0) {
      showToast({
        ...
        message: 'Please select at least 1 Primary Focus Area.',
        type: 'warning'
      });
      return;
    }
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-1">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[10px] font-extrabold uppercase tracking-wider bg-[#e6f6e8] text-[#2FA137] px-3 py-1 rounded-full border border-emerald-200/60">
                  {currentUser.role ? currentUser.role.replace(/_/g, ' ') : 'Verified Member'}
                </span>
              ...
              <h1 className="text-2xl sm:text-3xl font-black text-[#060721]">
                {currentUser.name || 'George Ikechukwu'}
              </h1>
              <p className="text-xs text-slate-600 font-medium flex items-center gap-1.5 flex-wrap">
                <span>{currentUser.dept || 'Faculty of Engineering (Mechanical)'}</span>
                <span>•</span>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0 w-full sm:w-auto">
            ...
            >
              ...
              <span>Upload Photo & Edit Profile</span>
            </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-4 border-t border-slate-100 text-xs">
          <div className="flex items-center gap-2 p-3 rounded-xl bg-slate-50 border border-slate-200/80">
            ...
            <div className="min-w-0">
              ...
              <span className="font-bold text-slate-800 truncate block">{currentUser.email || 'george.ikechukwu230233765@st.lasu.edu.ng'}</span>
            </div>
          </div>

          <div className="flex items-center gap-2 p-3 rounded-xl bg-slate-50 border border-slate-200/80">
            <User className="w-4 h-4 text-[#2FA137] shrink-0" />
            <div className="min-w-0">
              <span className="text-[10px] text-slate-400 font-bold uppercase block">Matriculation Number</span>
              <span className="font-bold text-slate-800 tracking-wider font-mono">{currentUser.matricNumber || '230233765'}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 p-3 rounded-xl bg-slate-50 border border-slate-200/80">
            ...
            <div className="min-w-0">
              ...
              <span className="font-extrabold text-[#2FA137]">Verified Student Account</span>
            </div>
          </div>
        </div>
      </div>

      {/* Grid Row: Skills & Focus Areas */}
          ...
          <div className="relative w-full max-w-2xl bg-white rounded-3xl p-6 sm:p-8 shadow-2xl border border-slate-200 my-auto space-y-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-black text-[#060721]">Edit Member Profile & Upload Photo</h3>
              </div>
              <button
            ...
            <form onSubmit={handleSaveProfile} className="space-y-5 text-xs">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                  ...
                  <div className="space-y-2 flex-1 w-full">
                    ...
                    <p className="text-[11px] text-slate-500 font-medium">
                      Select your photo file (JPEG, PNG, WEBP max 8MB). It will be saved directly to your profile.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block font-bold text-slate-700 mb-1">Faculty & Department *</label>
                  <input
                    type="text"
                    required
                    value={dept}
